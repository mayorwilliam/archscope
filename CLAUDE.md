...
- **Tests de DB viva** (`packages/db/test/live-postgres.test.ts`, `live-mysql.test.ts`): Testcontainers, gated por `TEST_LIVE_DB=1` (requiere Docker). Plantan drift exacto y validan que el reporte contenga eso y **nada más**, más la guarda de credenciales (grep de URL/password sobre todo output de `.archmap/`). El de MySQL además cubre el mapeo schema==database y `tinyint(1)`→boolean.
...
**Fase 6 (última) implementada — falta solo el publish**: extractores TypeORM/Drizzle/Django (fixtures `typeorm-app`/`drizzle-app`/`django-app` con goldens revisados), introspección MySQL con mapeo de schema default, Alembic best-effort, parse paralelo con piscina (umbral 200 misses; bench 5k archivos: ~3s frío / ~2s caliente, presupuesto 60s/5s, determinismo verificado por hash), bundle tsup de un solo paquete + smoke real de `npm pack` → `npx archmap init && analyze && serve && mcp` en proyecto limpio, CI multiplataforma (`.github/workflows/ci.yml`), README + CHANGELOG 0.1.0.

**Bloqueantes del publish v0.1.0**: (1) el nombre `archmap` YA está tomado en npm (existe `archmap@1.5.0`) — hay que decidir nombre definitivo (el plan siempre lo trató como working name) y renombrar bin/paquete/docs; (2) `npm login` del usuario; (3) no hay remote de GitHub configurado (el CI está escrito pero nunca corrió; el campo `repository` del package.json falta a propósito).
