...
**Bloqueantes del publish v0.1.0**: (1) `npm login` del usuario; (2) no hay remote de GitHub configurado (el CI está escrito pero nunca corrió; el campo `repository` del package.json falta a propósito). Antes del publish: repetir el smoke de `npm pack` → `npx archscope` con el nombre nuevo.
